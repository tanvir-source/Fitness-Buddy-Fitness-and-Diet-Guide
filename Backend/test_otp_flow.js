...
async function testFlow() {
    console.log(`\n🔹 Testing OTP Password Reset Flow for: ${email}`);
    ...
    console.log('🔑 Dev OTP:', forgotData.dev_token);

    if (!forgotData.dev_token) {
        console.error('❌ No dev OTP received.');
        return;
    }
    ...
    const resetRes = await fetch(`${API_URL}/reset-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, otp: forgotData.dev_token, newPassword })
    });
